...
TEST(String, CopyConstructor) {
  ...
  ASSERT_EQ(1u, s2.nr_copies());

  ASSERT_EQ(7u, s1.size());
  ASSERT_STREQ("string1", s1.c_str());
  ASSERT_EQ(0u, s1.nr_copies());
}
...
// Test swap method with move instead of copy
...
// Test BubbleSort with move instead of copy
